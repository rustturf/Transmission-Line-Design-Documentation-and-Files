...
\begin{align}
    ...
    &=479.96 cm\notag
\end{align}
The figure of a three phase double circuit transmission tower is as shown below: 
\begin{figure}[!hb]
    \centering
    \includegraphics[width=0.5\textwidth,height=0.9\textwidth]{Figures/transmissiontower.pdf}
    \caption{Transmission Tower of a Three Phase Double Circuit Transmission Line}
    \label{fig:Transmission Tower}
\end{figure}
...
\begin{figure}[!hb]
    \centering
    \includegraphics[width=0.6\textwidth,angle=-270]{Figures/GMDD.pdf}
...
\end{align*}
From equation \ref{equation:GMD}:
\begin{align*}
    GMD=\sqrt[3]{5.7596*5.7596*7.7939}=6.370m
\end{align*}
\textbf{GMR Calculation}\\[0.2cm]
We know:\\
    $\text{GMR of self conductor}(r')=0.7788r=0.7788*0.012985m=0.0101m$\\[0.2cm]
The GMR in a three-phase double circuit line is given by: 
    \begin{equation}
        GMR_L=\sqrt[3]{GMR_{R_L}*GMR_{Y_L}*GMR_{B_L}}
        \label{equation:GMRL}
    \end{equation}
where,
\begin{align*}
    GMR_{R_L}&=\sqrt[4]{D_{RR}*D_{RR'}*D_{R'R}*D_{R'R'}}=\sqrt[4]{r'*D_{RR'}*D_{R'R}*r'}=0.3142m\\
    GMR_{Y_L}&=\sqrt[4]{D_{YY}*D_{YY'}*D_{Y'Y}*D_{Y'Y'}}=\sqrt[4]{r'*D_{YY'}*D_{Y'Y}*r'}=0.2897m\\
    GMR_{B_L}&=\sqrt[4]{D_{BB}*D_{BB'}*D_{B'B}*D_{B'B'}}=\sqrt[4]{r'*D_{BB'}*D_{B'B}*r'}=0.3142m
\end{align*}
Thus from equation \ref{equation:GMRL}:
\begin{align*}
    GMR_L=\sqrt[3]{0.3142*0.2897*0.3142}=0.3058m
\end{align*}
To calculate $GMR_C,\ r'=r=0.012985m$ and we repeat the same process again:
\begin{equation}
    GMR_C=\sqrt[3]{GMR_{R_c}*GMR_{Y_c}*GMR_{B_c}}
    \label{equation:GMRC}
\end{equation}
where,
\begin{align*}
    GMR_{R_c}&=\sqrt[4]{D_{RR}*D_{RR'}*D_{R'R}*D_{R'R'}}=\sqrt[4]{r*D_{RR'}*D_{R'R}*r}=0.3563m\\
    GMR_{Y_c}&=\sqrt[4]{D_{YY}*D_{YY'}*D_{Y'Y}*D_{Y'Y'}}=\sqrt[4]{r*D_{YY'}*D_{Y'Y}*r}=0.3285m\\
    GMR_{B_c}&=\sqrt[4]{D_{BB}*D_{BB'}*D_{B'B}*D_{B'B'}}=\sqrt[4]{r*D_{BB'}*D_{B'B}*r}=0.3563m
\end{align*}
Thus from equation \ref{equation:GMRC}:
\begin{align*}
    GMR_C=\sqrt[3]{0.3563*0.3285*0.3563}=0.3467m
\end{align*}
\subsubsection{Inductance And Capacitance Calculation}
...
\begin{equation}
    Capacitance(C)=2\pi\epsilon_o \left(\frac{1}{ln\frac{GMD}{GMR_C}}\right)
    \label{C}
\end{equation}
From equation \ref{L} and \ref{C}:
\begin{align*}
    L&=2*10^{-7}*ln\left(\frac{6.370}{0.3058}\right)=6.072*10^{-7}H/phase\ m=6.072*10^{-4}H/phase\ Km\\
    C&=2\pi \epsilon_o*\left(\frac{1}{ln\frac{6.370}{0.3467}}\right)=1.910*10^{-11}F/phase\ m=1.910*10^{-8}F/phase \ km
\end{align*}
The total inductance and capacitance is therefore:
\begin{itemize}
    \item $L_{total}=L*110=0.066792H/phase$
    \item $C_{total}=C*110=2.101*10^{-6}F/phase$
\end{itemize}
\section{Voltage Regulation}
\subsection{Calculation Of ABCD parameters}
The sending end voltage and current can be related to the receiving end voltage and current as:
{\renewcommand{\arraystretch}{1.3} % Cell height scalings
\begin{equation}
    \begin{bmatrix}
        V_S\\
        I_S
...
\begin{equation*}
    ...
    \begin{bmatrix}
        1+\frac{YZ}{2}&Z\\
        Y\left(1+\frac{YZ}{4}\right)&1+\frac{YZ}{2}
    \end{bmatrix}
\end{equation*}
}\\[0.2cm]
The equivalent $\pi$ model of a transmission line as shown below:
\begin{figure}[!hb]
    \centering
    \includegraphics[width=1\textwidth]{Figures/figure3.jpg}
    \caption{$\pi$ model of a transmission line}
\end{figure}\\
We know,
\begin{align}
Z &= R_{65}+jX_L \label{eq:impedance} \\
  &= R_{65}+j(2\pi f L_{total}) \notag \\
  &= 11.56518 + j(2\pi\cdot 50 \cdot 0.066792) \notag \\
  &= 11.565 + 20.983j =23.959\angle 61.13^\circ \notag \\[6pt]
Y &= j\omega C_{total} \label{eq:admittance} \\
  &= j 2\pi f C_{total} \notag \\
  &= 2\pi \cdot 50 \cdot 2.101 \times 10^{-6} j \notag \\
  &= 6.600 \times 10^{-4} j\ \Omega^{-1} \notag
\end{align}

For the $\pi$ model circuit:
\begin{align*}
    A&=D=1+\dfrac{ZY}{2}=1+\dfrac{(11.565+20.983j)(6.600*10^{-4}j)}{2}=0.9930\angle 0.220^\circ\\
    C&=Y\left(1+\frac{YZ}{4}\right)=6.600*10^{-4}\left(1+\dfrac{(11.565+20.983j)(6.600*10^{-4}j)}{4}\right)\\
    &=6.5771*10^{-4}\angle 90.10^\circ
\end{align*}
From equation \ref{vsequn}:
\begin{align*}
V_S &= A V_R + B I_R=(0.9930\angle 0.220^\circ) V_R+(23.959\angle 61.13^\circ)I_R\\
I_S &= C V_R + D I_R =(6.5771*10^{-4}\angle 90.10^\circ)V_R+(0.9930\angle 0.220^\circ)I_R\\
V_{R(FL)}&=\dfrac{132}{\sqrt{3}}\angle 0^\circ=76.210\angle 0^\circ \ KV\\
I_R&=323.1\angle -cos^{-1}(0.9)=323.1\angle(-25.84^\circ)\ A
\end{align*}
The full load Source Voltage from equation \ref{vsequn} is given by:
\begin{align*}
V_{S(FL)}&=(0.9930\angle 0.220^\circ)*(76.210\angle 0^\circ)+(23.959\angle 61.13^\circ)*323.1\angle(-25.84^\circ)\ \\
&=82.132\angle 3.324^\circ
\end{align*}
The negative sign in the current denotes that the line is operating at a lagging power factor.\\
At no load $I_R=0$:
\begin{align*}
    V_{R(NL)}&=\frac{V_{S(FL)}}{A}\\
    &=\frac{82.132\angle 3.324^\circ}{(0.9930\angle 0.220^\circ)}\\
    &=82.71\angle3.104\ KV
\end{align*}
The voltage regulation of the conductor is given by: 
\begin{align}
    \textit{Voltage Regulation}(V_R)&=\dfrac{|V_{R(NL)}|-|V_{R(FL)}|}{|V_{R(FL)}|}\times 100\%\\
    &=\dfrac{82.71-76.21}{76.21}\times 100\%\notag\\
    &=8.52 \%\notag
\end{align}
Since the voltage regulation is less than 12\% and efficiency is greater than 94\% both the transmission efficiency and voltage regulation criteria is satisfied.
